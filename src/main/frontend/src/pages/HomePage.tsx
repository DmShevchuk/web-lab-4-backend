...
export default function HomePage() {
    ...
    function loadNewPoints(page: number, showOwned: boolean) {
        ...
        .unwrap()
        .then((resp) => {
            if (resp.number >= resp.totalPages && resp.number !== 0) {
                loadNewPoints(0, showOwned);
            }
        }) 
        .catch((err) => {
            dispatch(warningToast(`Не удалось обновить страницу: ${err}`));
        })
    }
    ...
    function submitPoints(request: CompoundPointRequest) {
        ...
        .then((result) => {
            if (result.success) {
                dispatch(successToast(`Все точки успешно отправлены!`));
                loadNewPoints(currentPage, onlyOwned);
            } else {
                dispatch(warningToast(`Не удалось отправить точки!`));
            }
    ...
    return (
        <div>
            ...
            <div className='bg-gray-100 w-fit h-fit p-3 rounded-xl mx-auto mb-5 shadow-xl'>
