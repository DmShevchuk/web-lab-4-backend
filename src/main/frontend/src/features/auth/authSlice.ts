...
export function registerAndAuthenticateUser(login: string, firstName: string, lastName: string, password: string) :AppThunk<Promise<ApiCallStatus<string>>> {
    return async function(dispatch, getState) {
        // if (result.success) {
        //     const sub = jwtDecode<JwtPayload>(result.payload.accessToken).sub;
        //     if (sub === undefined) return {success: false, message: 'Invalid token', payload: {accessToken:'', refreshToken:''}};
        ...
        return await registerUser(login, firstName, lastName, password);
    } 
}
